import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */import{D as f}from"./database-CTJo1PQf.js";import{C as y}from"./cpf-validator-B4PsRAE6.js";import{N as h}from"./navigation-BwbyOJod.js";class r{static showLoadingNotification(){const e=document.createElement("div");e.id="trackingNotification",e.style.cssText=`
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        `,document.body.appendChild(t),setTimeout(()=>{t.parentNode&&t.remove()},5e3)}addPostDeliveryPaymentSteps(e,t){const o=document.getElementById("trackingTimeline");if(!o)return;const a=this.getStageNames();let i=[];t===1?i=[17,18,19]:t===2?i=[21,22,23]:t===3&&(i=[25,26]);const n=[0,2*60*1e3,2*60*60*1e3];i.forEach((s,u)=>{const l=n[u]||0;setTimeout(()=>{const p=new Date,d=this.createTimelineItem({id:s,date:p,title:a[s],description:a[s],isChina:!1,completed:!0,needsLiberation:!1,needsDeliveryPayment:s===20||s===24,deliveryAttempt:s===20?2:s===24?3:null},u===i.length-1);o.appendChild(d),setTimeout(()=>{d.style.opacity="1",d.style.transform="translateY(0)"},100),d.scrollIntoView({behavior:"smooth",block:"center"}),(s===20||s===24)&&setTimeout(()=>{this.highlightDeliveryButton(s)},1e3)},l)})}highlightDeliveryButton(e){const t=document.querySelector(`[data-step-id="${e}"]`);t&&(r.scrollToElement(t,window.innerHeight/2),setTimeout(()=>{t.style.animation="pulse 2s infinite, glow 2s ease-in-out",t.style.boxShadow="0 0 20px rgba(30, 74, 107, 0.8)",setTimeout(()=>{t.style.animation="pulse 2s infinite",t.style.boxShadow="0 4px 15px rgba(30, 74, 107, 0.4)"},6e3)},500))}closeDeliveryPaymentModal(){const e=document.getElementById("deliveryPaymentModal");e&&(e.style.animation="fadeOut 0.3s ease",setTimeout(()=>{e.parentNode&&e.remove(),document.body.style.overflow="auto"},300))}addPaymentSimulationButton(){const e=document.querySelector(".professional-modal-content");if(!e||document.getElementById("simulatePaymentButton"))return;const t=document.createElement("div");t.style.cssText=`
            margin-top: 20px;
            padding: 15px;
